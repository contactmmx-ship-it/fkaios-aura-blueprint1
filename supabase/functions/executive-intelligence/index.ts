...
// Executive Intelligence Upgrade (governed_decisions: see FKAIOS_EIL_UPGRADE governance
// record) — folds cognitiveTick()'s unique capabilities into this already-live daily
// cycle instead of deploying a second, competing daily brain loop (founder-brain-tick).
import { imagine, assessRisk, simulateStrategies } from "../_shared/founder-brain.ts";
import { curiosityTick } from "../_shared/curiosity.ts";
// Founder Decision Intelligence (v0.9.3 engine, wired in v0.9.4): read-only
// analysis over the Founder Decision Memory Loop (fleet_memory,
// memory_type='decision') — surfaced here as its own labeled observation,
// distinct from the generic organizational_memory dump, so the model reasons
// over founder-approval patterns as a first-class signal.
import { generateFounderDecisionProfile } from "../_shared/decision-intelligence.ts";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    const [milestones, leadsByStage, pendingApprovals, projects, trust, kpis, delegations, duePredictions, memory, board, execCommittee, orgUnits, founderDecisionProfile] = await Promise.all([
    ...
    const observed = { organizational_memory: memory, founder_decision_profile: founderDecisionProfile, org_structure: { board, executive_committee: execCommittee, org_units: orgUnits }, milestones, leadsByStage, pendingApprovals, projects, agent_trust: trust, latest_kpis: kpis, open_delegations: delegations, predictions_due: duePredictions, observed_at: new Date().toISOString() };
    ...
    // GOVERNANCE UPGRADE: reuse founder-brain.ts's assessRisk() (the same
    // classifier cognitiveTick uses) instead of the prior unconditional
    // requires_founder_approval:false. LOW stays autonomous; MEDIUM is tagged
    // for review via the existing (previously-unused) context jsonb column,
    // still non-blocking; HIGH/CRITICAL flips requires_founder_approval:true,
    // which FounderStory.tsx already surfaces as a real "needs you" item.
    const directives = Array.isArray(exec.directives) ? exec.directives.slice(0, 4) : [];
    for (const d of directives) {
      const riskLevel = await assessRisk(String(d.task ?? ""), cycleCorrelationId);
      await supabase.from("agent_task_delegations").insert({
        from_agent: "executive-intelligence",
        to_agent: String(d.to_agent ?? "unassigned"),
        task_description: String(d.task ?? ""),
        context: { risk_level: riskLevel },
        requires_founder_approval: riskLevel === "high" || riskLevel === "critical",
      });
    }

    // STRATEGIC INTELLIGENCE: before staging a capital-allocation approval,
    // reuse founder-brain.ts's simulateStrategies() to generate and score
    // alternatives, so the founder sees what was considered and rejected —
    // not just the one recommendation.
    const capital = Array.isArray(exec.capital_allocation) ? exec.capital_allocation.slice(0, 2) : [];
    const capitalBriefingNotes: string[] = [];
    for (const c of capital) {
      if (c && (c.amount_inr ?? null) !== null) {
        let rejectedAlternatives: Array<{ description: string; predictedOutcome: string; score: number }> = [];
        try {
          const sim = await simulateStrategies(
            "founder",
            `Capital allocation candidate: ${c.recommendation}. Amount: INR ${c.amount_inr}. Rationale: ${c.why}`,
            3,
            cycleCorrelationId,
          );
          rejectedAlternatives = sim.rejected;
          if (sim.selected) {
            capitalBriefingNotes.push(`"${c.recommendation}" (INR ${c.amount_inr}): ${sim.rejected.length} alternative(s) considered and rejected (top rejected: ${sim.rejected[0]?.description ?? "none"}).`);
          }
        } catch (err) {
          console.error("executive-intelligence: simulateStrategies for capital allocation failed (non-blocking)", err instanceof Error ? err.message : String(err));
        }
        await supabase.from("approvals").insert({
          action_type: "executive_capital_allocation", risk_level: "high", amount_inr: Number(c.amount_inr),
          reason: `${c.recommendation}: ${c.why}`.slice(0, 900),
          status: "pending", department_code: "FOUNDER",
          payload: { source: "executive-intelligence", rejected_alternatives: rejectedAlternatives },
        });
      }
    }

    const founderBriefing = capitalBriefingNotes.length > 0
      ? `${String(exec.founder_briefing ?? "")}\n\nStrategic alternatives considered for capital allocation:\n${capitalBriefingNotes.join("\n")}`
      : String(exec.founder_briefing ?? "");
    ...
    const memoryWrites: PromiseLike<unknown>[] = [];
    ...
    // imagine() and curiosityTick() persist their own findings into the same
    // fleet_memory substrate this cycle already writes to — no separate write here.
    let imaginationText = "";
    try {
      imaginationText = await imagine(
        "founder",
        `Given this executive cycle's situation assessment, what's one non-obvious thing worth trying that hasn't been tried?\n\n${String(exec.situation_assessment ?? "")}`,
        cycleCorrelationId,
      );
    } catch (err) {
      console.error("executive-intelligence: imagine() failed (non-blocking)", err instanceof Error ? err.message : String(err));
    }

    let curiosityResults: Awaited<ReturnType<typeof curiosityTick>> = [];
    try {
      curiosityResults = await curiosityTick("founder", cycleCorrelationId);
      console.log(JSON.stringify({ level: "INFO", message: "executive-intelligence: curiosityTick completed", results: curiosityResults.length, source: "executive-intelligence" }));
    } catch (err) {
      console.error("executive-intelligence: curiosityTick() failed (non-blocking)", err instanceof Error ? err.message : String(err));
    }

    return j({
      success: true, cycle_id: cycle.id, cycle_number: cycle.cycle_number, directives: directives.length, capital_staged: capital.length,
      predictions: preds.length, memories_recorded: memoryWrites.length, founder_briefing: founderBriefing,
      imagined: imaginationText, curiosity_results: curiosityResults.length,
    });
  } catch (err) {
